...
def load_deeplab_model():
    ...
    model = network.modeling.__dict__['deeplabv3plus_resnet101'](num_classes=21, output_stride=8)
    model.load_state_dict(torch.load("best_deeplabv3plus_resnet101_voc_os16.pth", weights_only=False)['model_state'])
...
def apply_variable_blur(image, bbox, segmentation_mask, light_blur_std=40, strong_blur_std=110):
    ...
    intersection_mask = Image.new("L", image_pil.size, 0) 

    draw = ImageDraw.Draw(bbox_mask)
    draw.rectangle([x1, y1, x2, y2], fill=255)

    seg_pixels = segmentation_mask.nonzero()
...
class Executor:
    def __init__(self, args=None, device: str = "cpu", box_representation_method: str = "crop", method_aggregator: str = "max", enlarge_boxes: int = 0, expand_position_embedding: bool = False, square_size: bool = False, blur_std_dev: int = 100, cache_path: str = None) -> None:
        IMPLEMENTED_METHODS = ["crop", "blur", "shade", "fine-grained", "multilayer_Blur"] 
        ...
        self.device = device
        self.deep_model = load_deeplab_model().to(self.device).eval()
    ...
    def tensorize_inputs(self, caption: str, image: Image, boxes: List[Box], image_name: str = None) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        if self.cache_path is None or any([not os.path.exists(os.path.join(self.cache_path, model_name, image_name, method_name+".pt")) for model_name in self.model_names for method_name in self.box_representation_method.split(',')]):
            ...
            if "multilayer_Blur" in self.box_representation_method:
                image_tensor, _ = preprocess_image(image)
                image_tensor = image_tensor.to(self.device)
                segmentation_mask = segment_image(self.deep_model, image_tensor)
                ...
                for i in range(len(boxes)):
                    ...
                    blurred_image = apply_variable_blur(image, box, segmentation_mask)

                    preprocessed_images = self.preprocess_image(blurred_image)
                    for j, img in enumerate(preprocessed_images):
                        images[j].append(img.to(self.device))
